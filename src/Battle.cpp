...
void Battle::assign_BattleLog(Player p1, Player p2, vector<Figment> figmentlist)
{
	...
	Character* tylor = &p1;
	Character* liza = &p2;	
	battlelog.insert(pair<int, Character*>(tylor->get_Haste(), tylor));
	battlelog.insert(pair<int, Character*>(liza->get_Haste(), liza));
	...
	for (unsigned int i = 0; i < figmentlist.size(); ++i)
	{
		...
		battlelog.insert(pair<int, Character*>(figment->get_Haste(), figment));
	}	
}
